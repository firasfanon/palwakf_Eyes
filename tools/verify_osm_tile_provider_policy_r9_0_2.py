...
def snapshot_bytes(relative: str) -> tuple[bytes | None, str | None]:
    completed = subprocess.run(
        [
            "git",
            "show",
            f"{MANIFEST_SNAPSHOT_COMMIT}:{relative}",
        ],
        cwd=ROOT,
...
def manifest_byte_candidates(data: bytes) -> list[bytes]:
    ...
    # Historical R9 manifest was generated from a Windows worktree.
    # Git stores canonical LF text because .gitattributes enforces LF.
    # Accept only exact LF/CRLF materializations of identical content.
...
def verify_manifest() -> tuple[bool, list[str]]:
    ...
    # The manifest itself must remain semantically identical to the
    # manifest committed in the immutable R9 checkpoint.
    snapshot_manifest, snapshot_error = snapshot_bytes(
    ...
    if snapshot_manifest is None:
        failures.append(
            ...
            f"{MANIFEST_SNAPSHOT_COMMIT}: "
            f"{snapshot_error}"
        )
    else:
        try:
            snapshot_payload = json.loads(
                snapshot_manifest.decode("utf-8-sig")
            )
        except (UnicodeDecodeError, json.JSONDecodeError) as error:
            failures.append(
                f"snapshot manifest parse error: {error}"
            )
        else:
            if snapshot_payload != payload:
                failures.append(
                    "current manifest differs from pinned R9 snapshot"
                )
    ...
    for entry in files:
        ...
        if digest_match and size_match:
            failures.append(
                "manifest digest/size pair mismatch: "
                f"{relative}"
            )
